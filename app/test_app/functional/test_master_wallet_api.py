...
def test_create_bulk_credit_transfer(test_client, authed_stengo_admin_user, data, expected_response_code, expected_response):
    authed_stengo_admin_user.default_organisation.org_level_transfer_account._balance_offset_wei = int(100e18)
    authed_stengo_admin_user.default_organisation.org_level_transfer_account.update_balance()
    # Create admin user and auth
    authed_stengo_admin_user.set_held_role('ADMIN', 'superadmin')
    ...
    if response.status_code != 201:
        ...
        assert response.json['data']['credit_transfer'][0]['sender_transfer_account']['id'] == authed_stengo_admin_user.default_organisation.org_level_transfer_account.id