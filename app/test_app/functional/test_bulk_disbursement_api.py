...
def test_prep_bulk_disbursement_api(test_client, complete_admin_auth_token, create_organisation):
    ...
    def make_new_user_and_token(name='francine', tier='superadmin'):
        ...
        user.create_admin_auth(email=name+'@withstengo.com', password='CalicoCat', tier=tier, organisation=org)
    ...
    _, sample_token_5 = make_new_user_and_token('approver', 'stengoadmin')
    _, sample_token_6 = make_new_user_and_token('approver', 'stengoadmin')
...
@pytest.mark.parametrize("include_list, disbursement_amount, expected_recipient_count, expected_total_disbursement_amount\
    ...
    ([5], 50, 1, 50, 201, ['Francine'], True, ['approver@withstengo.com'], ['PARTIAL', 'PARTIAL', 'PARTIAL', 'APPROVED', 'error']), 
])
...
@pytest.mark.parametrize("include_list, disbursement_amount, expected_recipient_count, expected_total_disbursement_amount\
    ...
    # Require multi-approval is on, stengodamin can still approve right away
    ([5], 50, 1, 50, 201, ['Francine'], True, [], ['APPROVED', 'error']), 
    # Require multi-approval is on, approvers list is in place, stengodamin can still approve right away
    ([5], 50, 1, 50, 201, ['Francine'], True, ['someuser'], ['APPROVED', 'error']), 
])
def test_stengoadmin_can_approve_anything(include_list, disbursement_amount, expected_recipient_count, expected_total_disbursement_amount,\
    response_status, recipient_firstnames, multi_approval_setting, allowed_approvers_setting, statuses, test_client, complete_admin_auth_token\
    , create_organisation):
    """
    Checks that stengoadmin can unilaterally approve a disbursement
    """
    global sample_token_5
    global sample_token_6
    ...
    assert response.status_code == response_status
    if response_status == 201:
        # Make stengoadmin 1, approve the first time
        resp_id = response.json['data']['disbursement']['id']
        phase_two_response = test_client.put(f'/api/v1/disbursement/{resp_id}',
            headers=dict(
        ...
        # Make stengoadmin 2, approve the second time
        resp_id = response.json['data']['disbursement']['id']
        phase_two_response = test_client.put(f'/api/v1/disbursement/{resp_id}',
            headers=dict(
