...
def create_organisation(test_client, init_database, external_reserve_token, create_master_organisation):
    from server.models.organisation import Organisation
    organisation = Organisation(name='stengo', token=external_reserve_token, default_disbursement=400, country_code='US')
...
@pytest.fixture(scope='module')
def new_stengo_admin_user(test_client, init_database, create_organisation):
    from server.models.user import User
    user = User()
    user.create_admin_auth(email='tristan@withstengo.com', password='TestPassword', tier='stengoadmin')
...
@pytest.fixture(scope='module')
def authed_stengo_admin_user(new_stengo_admin_user):
    """
    Returns a stengo admin user that is activated and has TFA set up
    """

    new_stengo_admin_user.is_activated = True
    new_stengo_admin_user.set_TFA_secret()
    new_stengo_admin_user.TFA_enabled = True


    return new_stengo_admin_user

@pytest.fixture(scope='module')
def activated_stengo_admin_user(new_stengo_admin_user):
    """
    Returns a stengo admin user that is activated but does NOT have TFA set up
    """
...
@pytest.fixture(scope='function')
def complete_admin_auth_token(authed_stengo_admin_user):
    return get_complete_auth_token(authed_stengo_admin_user)
...
@pytest.fixture(scope='function')
def create_blacklisted_token(authed_stengo_admin_user):
    from server.models.blacklist_token import BlacklistToken
    auth_token = authed_stengo_admin_user.encode_auth_token().decode()
    blacklist_token = BlacklistToken(token=auth_token, user_id=authed_stengo_admin_user.id)
...
@pytest.fixture(scope='function')
def create_ip_address(authed_stengo_admin_user):
    from server.models.ip_address import IpAddress
    ip_address = IpAddress(ip="210.18.192.196")
    ip_address.user = authed_stengo_admin_user
...
@pytest.fixture(scope='module')
def admin_with_org_reserve_balance(authed_stengo_admin_user, external_reserve_token, loaded_master_wallet_address):
    from server import bt

    amount = 100

    org_transfer_account = authed_stengo_admin_user.default_organisation.org_level_transfer_account
    ...
    return authed_stengo_admin_user
...
def loaded_master_wallet_address(load_account):
    """
    A blockchain address that isn't tracked in the stengo system like a regular one. Used during testing
